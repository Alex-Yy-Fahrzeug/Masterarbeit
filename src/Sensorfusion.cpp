		...
		if (rList_Fusion_Start.Objects[i].ProbOfExist == 7) {
			if (abs(rList_Fusion_Start.Objects[i].x(1)) < p_y_relzone_Radar) { //add RadarObjects within the relevant zone
				...
				for (UINT8 j = 0; j < mList_Fusion.n; j++) {
					...
					dX_max = max(Delta_p_x_max_abs, Delta_p_x_max_percent * mList_Fusion.Objects[j].x(0));
...
void fuseRadarObjects(UINT8 i)
{
	...
	RadObj_i->ProbOfExist = max(RadObj_i->ProbOfExist, RadObj_j->ProbOfExist);
	...
	RadObj_i->Width = max(RadObj_i->Width, RadObj_j->Width);
	RadObj_i->Length = RadObj_j->x(0) - RadObj_i->x(0) + RadObj_j->Length;
	RadObj_i->RCS = max(RadObj_i->RCS, RadObj_j->RCS);
}
...
void createMeasurementList() {
	...
	for (radID = 0; radID < rList_Fusion_Final.n; radID++) {
		...
		for (mobID = 0; mobID < mList_Fusion.n; mobID++) {
			...
			dX_max = max(Delta_p_x_max_percent * mList_Fusion.Objects[mobID].x(0), Delta_p_x_max_abs);
...
{
	memset(&UDPObjects, 0, sizeof(UDPObjects));
	NofObjects_UDP = min(MAX_UDP_OBJECTS, fList_Fusion.n);
...
void send_UDP_MeasurementList() {
	memset(&UDPObjects, 0, sizeof(UDPObjects));
	NofObjects_UDP = min(MAX_UDP_OBJECTS, fList_Measurement.n);
...
void send_UDP_MobileyeList() {
	memset(&UDPObjects, 0, sizeof(UDPObjects));
	NofObjects_UDP = min(MAX_UDP_OBJECTS, mList_Fusion.n);
...
void send_UDP_RadarList() {
	memset(&UDPObjects, 0, sizeof(UDPObjects));
	NofObjects_UDP = min(MAX_UDP_OBJECTS, rList_Fusion_Start.n);
...
void send_UDP_ObjectList() {
	...
	NofObjects_UDP = min(fList_Fusion.n,(UINT8)40);
...
void Remove_expired()
{
	...
	for (int i = 0; i < Point_Buffer.n; ++i)
	{
		...
		bool  expiration_2 = (Point_Buffer.Object_History[i].NumofPoint <= 5) && ((timestamp_now - Point_Buffer.Object_History[i].timestamp_latest_renew) > 2.0);
		bool  expiration_3 = (timestamp_now - Point_Buffer.Object_History[i].timestamp_latest_renew) > 5.0;
...
void Fit_curve()
{
	...
	float cur_nonaim = 0;
	float cur_aim = NAN;
	float cur_vehicle = 0;     //to be readed from MAB

	for (int i = 0; i < Point_Buffer.n; ++i)
	{
		if (Point_Buffer.Object_History[i].NumofPoint > 5)
		{
			float sum_of_x_2p = 0;   // Summe of square of x
			float sum_of_x_4p = 0;   // Summe of 4th power of x
			float sum_of_y = 0;      // Summe of y
			float sum_of_y_x_2p = 0; // Summe of square of x multiply y
			float n = 0;

			for (int j = 0; j < Point_Buffer.Object_History[i].NumofPoint; ++j)
			{
				++n;
				sum_of_x_2p += pow(Point_Buffer.Object_History[i].Position[0][j], 2.0f);
				sum_of_x_4p += pow(Point_Buffer.Object_History[i].Position[0][j], 4.0f);
				sum_of_y += Point_Buffer.Object_History[i].Position[1][j];
				sum_of_y_x_2p += Point_Buffer.Object_History[i].Position[1][j] * pow(Point_Buffer.Object_History[i].Position[0][j], 2.0f);
			}
			Eigen::Matrix2f A;
			Eigen::Vector2f B;
			A << sum_of_x_4p, sum_of_x_2p,
				sum_of_x_2p, n;
			B << sum_of_y_x_2p,
				sum_of_y;
			Eigen::Matrix2f C;
			C << 1, 0,
				0, 0;
			Eigen::Vector2f cur = ((A + 0 * C).inverse() * B);
			Point_Buffer.Object_History[i].curvature[0] = cur[0];
			Point_Buffer.Object_History[i].curvature[1] = cur[1];
			if (std::abs(Point_Buffer.Object_History[i].curvature[0]) > 0.1) Point_Buffer.Object_History[i].curvature[0] = INFINITY;
	...
	float cur_tem = cur_ego;
	// Then the curvature of the perdicted trajectory will be coculated with weight.
	if (!isnan(cur_aim) && !isinf(cur_aim))
	{
		if (count_nonaim != 0)
		{
			cur_ego = (0.6f * cur_aim + 0.3f * cur_nonaim / count_nonaim + 0.1f * cur_vehicle);
		}
		else
		{
			cur_ego = (0.8f * cur_aim + 0.2f * cur_vehicle);
		}
	}
	else
	{
		if (count_nonaim != 0)
		{
			cur_ego = (0.7f * cur_nonaim / count_nonaim + 0.3f * cur_vehicle);
		}
		else
		{
			cur_ego = (cur_vehicle);
	...
	cur_ego = 0.2 * cur_tem + 0.8 * cur_ego;
}
...
void SPW_distribution()
{
	y_feld.row(8) = x_feld.row(8) * x_feld.row(8) * cur_ego;
	Eigen::ArrayXXf d_feld(1, 41);
	d_feld = (y_feld.row(8).square() + x_feld.row(8).square()).sqrt();

	for (UINT8 i = 0; i < 8; i++)
	{
		Eigen::ArrayXXf width(1, 41);
		width = width_coefficient(7 - i, 0) * d_feld.square() + width_coefficient(7 - i, 1) * d_feld + width_coefficient(7 - i, 2);
		x_feld.row(i) = x_feld.row(8) - width * 2.0f * cur_ego * x_feld.row(8) / (4.0f * pow(cur_ego, 2) * (x_feld.row(8)).square() + 1).sqrt();
		y_feld.row(i) = y_feld.row(8) + width / (4.0f * pow(cur_ego, 2) * (x_feld.row(8)).square() + 1.0f).sqrt();
		x_feld.row(16 - i) = x_feld.row(8) + width * 2.0f * cur_ego * x_feld.row(8) / (4.0f * pow(cur_ego, 2) * (x_feld.row(8)).square() + 1).sqrt();;
		y_feld.row(16 - i) = y_feld.row(8) - width / (4.0f * pow(cur_ego, 2) * (x_feld.row(8)).square() + 1.0f).sqrt();
	}
}
...
void PLA_update()
{
	...
	for (int i = 0; i < Point_Buffer.n; ++i)
	{
		...
		Point_Buffer.Object_History[i].PLA = min(1.0f, Point_Buffer.Object_History[i].PLA);
		Point_Buffer.Object_History[i].PLA = max(0.0f, Point_Buffer.Object_History[i].PLA);
...
void Select_with_PLA()
{
	...
	bool tic2 = ((Aim_Info.Exist_of_Aim != 0) && Aim_Info.PLA < 0.2f);
	...
	{
		if (Aim_Info.Exist_of_Aim == 1 && Aim_Info.PLA > 0.4)
		{
			for (int i = 0; i < Point_Buffer.n; ++i)
			{
...
void ReceiveThread_MAB(void* Param)
{
	...
	while (true) {
		...
		if (UDP_Recivcount < 0)
		{
			...
			memcpy(&Fahrdynamik.Yaw_Rate, buf_MAB_Receive, 4);
			memcpy(&Fahrdynamik.Long_Vel, buf_MAB_Receive+4, 4);
			memcpy(&Fahrdynamik.Lat_Vel, buf_MAB_Receive+8, 4);
		}
	} 
}