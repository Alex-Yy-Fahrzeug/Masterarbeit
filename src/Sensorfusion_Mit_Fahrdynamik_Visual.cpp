...
void    weighted_ego_curve(float cur_aim, float cur_nonaim, float count_nonaim, float cur_vehicle);
...
void Fit_curve()
{
	...
	for (int i = 0; i < Point_Buffer.n; ++i)
	{
		if (Point_Buffer.Object_History[i].NumofPoint > 5)
		{
			...
			C << 1, 0,
				0, 0;
			Eigen::Vector2f cur = ((A + 3 * C).inverse() * B);
			...
			else
			{
				if (!isnan(Point_Buffer.Object_History[i].curvature[0]) && !isinf(Point_Buffer.Object_History[i].curvature[0]))
				{
					...
					{
						distance_max = std::max(distance_max, sqrt(pow(double(Point_Buffer.Object_History[i].Position[0][k]), 2) + pow(double(Point_Buffer.Object_History[i].Position[1][k]), 2)));
					}
					if (distance_max > 10)
					{
	...
	weighted_ego_curve(cur_aim, cur_nonaim, count_nonaim, cur_vehicle);
}
...
void weighted_ego_curve(float cur_aim, float cur_nonaim, float count_nonaim, float cur_vehicle)
{
	...
	if (!isnan(cur_aim) && !isinf(cur_aim))
	{
		if (count_nonaim != 0)
		{
			...
			cur_road = 0.7f * cur_aim + 0.3 * cur_nonaim;
			curve_Interpolation(cur_road, cur_vehicle);
		}
		else
		{
			cur_road = cur_aim;
...
void SPW_distribution()
{
	...
	for (int i = 0; i < 41; ++i)
	{
		y_feld(8, i) = x_feld(8, i) * x_feld(8, i) * cur_ego[i];
	}
	...
	for (int i = 0; i < 8; i++)
	{
		...
		for (int j = 0; j < 41; ++j)
		{
			x_feld(i, j) = x_feld(8, j) - width(0, j) * 2.0f * cur_ego[j] * x_feld(8, j) / sqrt(4.0f * pow(cur_ego[j], 2) * pow(x_feld(8, j), 2) + 1);
			y_feld(i, j) = y_feld(8, j) + width(0, j) / sqrt(4.0f * pow(cur_ego[j], 2) * pow(x_feld(8, j), 2) + 1);
			x_feld((Eigen::Index)16 - i, j) = x_feld(8, j) + width(0, j) * 2.0f * cur_ego[j] * x_feld(8, j) / sqrt(4.0f * pow(cur_ego[j], 2) * pow(x_feld(8, j), 2) + 1);
			y_feld((Eigen::Index)16 - i, j) = y_feld(8, j) - width(0, j) / sqrt(4.0f * pow(cur_ego[j], 2) * pow(x_feld(8, j), 2) + 1);
		}
	}
}
...
void Select_with_PLA()
{
	...
	for (int i = 0; i < Point_Buffer.n; ++i)
	{
		if (!(Point_Buffer.Object_History[i].ID_Mob == Aim_Info.Info.ID_Mob && Point_Buffer.Object_History[i].ID_Rad == Aim_Info.Info.ID_Rad))
		{
			PLA_max = std:: max(PLA_max, Point_Buffer.Object_History[i].PLA);
		}
	}
	...
	{
		if (Aim_Info.PLA > 0.2) //Aim_Info.Exist_of_Aim == 1 && Aim_Info.PLA > 0.4
		{
			for (int i = 0; i < Point_Buffer.n; ++i)
			{
	...
	if (Aim_Info.Exist_of_Aim)
	{
		for (int i = 0; i <= Point_Buffer.n; ++i)
		{
			if (Aim_Info.Info.ID_Mob == Point_Buffer.Object_History[i].ID_Mob && Aim_Info.Info.ID_Rad == Point_Buffer.Object_History[i].ID_Rad)
			{
				Point_Buffer.Object_History[i].Selected = 1;
			}
		}
	}
	Aim_Info.Detection = 0;
